...
export const fetchData = (url: string, params: Object, abortController: AbortController): Promise<Response | Error> => {
...
const useFetch = (url: string, getData: typeof fetchData, params: Object = {}) => {
  ...
  useEffect(() => {
    ...
    const asyncSetState = async () => {
      try {
        ...
        const data = await getData(url, params, abortController as AbortController);

        if (!isCanceled) {
          dispatch({ type: "resolved", data });
        }
      } catch (error) {
        if (!isCanceled && error.name !== "AbortError") dispatch({ type: "rejected", error });
